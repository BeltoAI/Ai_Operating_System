...
object AgentClient {
    ...
    fun ask(
        ...
        val system = buildString {
            ...
            append("Use send_photo when the user wants to FIND a photo from their own gallery/phone and optionally SEND it to someone — 'find a full-body photo of me and send it to my wife', 'text that beach picture to Sam', 'show me a selfie'. It searches your indexed photos + gallery and shares the best match; you do NOT need to see the image yourself. arg = {\"query\":\"what the photo shows, e.g. full body photo of me\",\"name\":\"recipient's name, or empty to just open it\",\"app\":\"whatsapp|telegram|sms|mail\",\"message\":\"optional caption\"}. NEVER say you can't access the gallery — use send_photo. ")
            ...
            append("invest / portfolio — use for \"how's my portfolio\", \"my stocks\", \"holdings\", \"buy/sell shares\". Opens the Trading screen. ")
            append("find_job={\"arg\":\"role or company\"} — \"find me a job at X\", \"apply for Y\". Opens the Job-hunt screen (resume + cover letter + outreach). ")
            append("shop={\"arg\":\"what they want\"} — \"find me the best X under Y\", \"where can I buy Z\". Opens Shopping with live web results. ")
            append("expenses — \"my spending\", \"how much did I spend\", \"my receipts\". Opens the Expenses screen with real tracked numbers. ")
            append("look — \"what am I looking at\", \"identify this\", \"scan this\". Opens the live camera. ")
            append("write_paper={\"arg\":\"topic\"} — \"write me a paper/report on X\". Opens Research. ")
            append("cowork — \"let's work on X together\". Opens a Cowork session. ")
            append("faces — questions about who is in a photo. operate — \"do this for me in the app\" (screen control). ")
            append("compose_post={\"arg\":\"topic\"} / spicy_post — write a social post. ")
            append("IMPORTANT: these OPEN A SCREEN. If the user clearly wants one, you MUST emit it — do not just describe what you would do. ")
            ...
            append("share_location={\"name\":\"Mom\",\"channel\":\"whatsapp|sms|telegram\",\"navigate\":false} — for 'share my location with X'. DEFAULT is a general share (no home): SMS/Telegram send a fresh Maps link on an interval; whatsapp opens a chat pre-filled with the current location to tap-send. channel: pick whatsapp if the user says WhatsApp, telegram if Telegram, else sms. Set 'navigate':true ONLY if the user ALSO asks to share 'until I'm home' or be navigated home — otherwise DO NOT set it (no home, no Maps). Never assume 'until home'. Add share_location to the action types. ")
    ...
    fun chat(prompt: String, memory: String = "", history: List<Pair<String, String>> = emptyList()): Pair<Int, String> {
        ...
        val sys = buildString {
            ...
            append("for longer answers. For ANY math, use LaTeX: inline as \$…\$ and display equations as \$\$…\$\$. ")
            append("Keep everyday replies concise; go deep when the question deserves it. ")
