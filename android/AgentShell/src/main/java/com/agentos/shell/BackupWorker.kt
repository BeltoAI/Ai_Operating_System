...
class BackupWorker(ctx: Context, params: WorkerParameters) : CoroutineWorker(ctx, params) {
    override suspend fun doWork(): Result {
        ...
        if (!BrainBackup.autoEnabled(ctx)) return Result.success()
        // Debounce: skip if we backed up within the last ~90 minutes (manual "Back up now" bypasses this).
        val force = inputData.getBoolean("force", false)
        if (!force && System.currentTimeMillis() - BrainBackup.lastBackup(ctx) < 90 * 60_000L) return Result.success()
        ...
        if (!force) {
            ...
            if (empty) return Result.success()
        }
